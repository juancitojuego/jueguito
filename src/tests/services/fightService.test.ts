...
} as jest.Mocked<IGameStateManager>;
...
describe('FightService', () => {
  ...
  beforeEach(() => {
    ...
    mockGameStateManager.updatePlayerActiveCombatEffects.mockImplementation((effects: any) => {
      currentGameStateEffects = effects;
    });
  ...
  describe('Round Workflow', () => {
    ...
    it('playerSelectsCard should update hand and discard pile via GameStateManager', async () => {
      ...
      expect(mockGameStateManager.addCardsToHand).toHaveBeenCalledWith([chosen]);
      expect(mockGameStateManager.addCardsToDiscardPile).toHaveBeenCalledWith(discarded);
    });
    ...
    it('resolveCurrentRound should calculate damage and update health', async () => {
      ...
      const powerBoostEffect = playerEffectsAfterRound.find((e: any) => e.name === 'Minor Power Boost');
