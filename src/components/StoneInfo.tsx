...
import { gameState, equippedStoneDetails } from '../store'; // Import new store items
...
const StoneInfo: Component = () => {
  return (
    <div>
      ...
      <Show
        when={equippedStoneDetails()} // Use the new derived memo
        fallback={<p>No stone equipped or details available.</p>}
      >
        {(stone) => ( // stone is the unwrapped StoneQualities object from the memo
      ...
      <hr />
      <div>
        <strong>Player:</strong> {gameState.playerStats.name || 'N/A'}
      </div>
      <div>
        <strong>Currency:</strong> ${gameState.currency}
      </div>
    </div>
  );
};
